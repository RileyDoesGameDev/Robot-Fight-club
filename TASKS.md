...
- [~] **T-8.2** Produce the deployable build: `build_export` / `build_bundle` via the engine-deployer "Deploy as WebGL" path; verify it runs from `:4173` in a clean browser profile. **(V)**
  ...
  - **2026-08-25 — packaged as a self-contained, shareable image.** `deploy/Dockerfile` bakes the whole game into unprivileged nginx: no host mount, no volume, no network at runtime, **27 MB**. One command to run (`docker compose -f deploy/docker-compose.yml up -d`), and `node tools/package-game.mjs --save` writes a `.tar` that anyone with Docker can `docker load` and play — verified by deleting the image, loading from the tarball and playing it.
  - `deploy/nginx.conf` gets the caching rules right rather than by default: `index.html`, `seed.js` and `bundle.json` are `no-store` because a stale copy of any of them produces a game that looks fine and behaves like an older version — the worst thing to hand somebody, because it does not look like a caching problem. gzip takes `player.js` from **4.9 MB to 1.5 MB**.
  - `package-game.mjs` **refuses to package a stale or unshimmed build**: it compares the build's mtime against every source under `game/` and checks the shim marker, because shipping a container that quietly runs last week's game is the failure that matters. Found and fixed a real bug doing this — `spawnSync` with `shell: true` concatenates argv, so the repo's own path (T-0.5, it has a space in it) broke `docker save` with an "Access is denied" that was actually a quoting error.
  - `deploy/README.md` is written for a recipient who has never seen this repo: controls for both players, how to play, and the two behaviours that look like bugs and are not — a backgrounded tab suspends the game entirely, and audio needs one click before it will start.
  - **Left open deliberately.** The deliverable is a build that stands on its own; this one stands on a patch to a vendored artifact that will break the moment the engine is rebuilt. The packaging above does not change that — it makes the shimmed build *distributable*, not *correct*. It closes when LIM-009 does, most likely by rebuilding the deployed player from current engine source, which is a build-pipeline change rather than an engineering one.
